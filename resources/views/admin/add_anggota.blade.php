            ...
            <form action="{{ route('admin-teams.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="content">
                    ...
                    <div class="kanan">
                        ...
                        is-invalid
                    @enderror" placeholder="Masukkan Nama Anggota" value="{{ old('name') }}">
                            @error('name')
                    ...
                    @enderror" name="position" id="position">
                                ...
                                <option value='Sekretaris 1' {{(old('position') === 'Sekretaris 1') ? 'selected' : ''}}>
                                    Sekretaris 1</option>
                                <option value='Sekretaris 2' {{(old('position') === 'Sekretaris 2') ? 'selected' : ''}}>
                                    Sekretaris 2</option>
                                <option value='Bendahara' {{(old('position') === 'Bendahara') ? 'selected' : ''}}>
                                    Bendahara</option>
                                <option value='Manager Unit Pangan'
                                    {{(old('position') === 'Manager Unit Pangan') ? 'selected' : ''}}>Manager Unit
                                    Pangan</option>
                                <option value='Manager Unit Barang & Jasa'
                                    {{(old('position') === 'Manager Unit Barang & Jasa') ? 'selected' : ''}}>Manager
                                    Unit Barang & Jasa</option>
                            </select>
                            @error('position')
                            <div class="invalid-feedback ml-4">
                                {{ $message }}
